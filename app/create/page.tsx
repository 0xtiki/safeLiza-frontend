...
// Dynamically import ReactJsonView
...
export default function Home() {
  ...
  const [useExistingPasskey, setUseExistingPasskey] = useState(true);
  ...
  const handleUseExistingPasskey = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/passkey/verify-signer`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
              ...
              <div className="bg-base-200 p-4 rounded-lg overflow-auto">
                {config.passkey?.name && (
                  <div className="flex gap-2 mb-2">
                    <div className="flex-1">
                      <span className="font-bold">passkey: {config.passkey ? '1' : '0'}</span>
                      {isClient && (
                        <ReactJsonView
                          src={{
                            name: config.passkey.name,
                ...
                {config.multisig && config.multisig.owners.length > 0 && (
                  <div className="flex gap-2 mb-2">
                    <div className="flex-1">
                      <span className="font-bold">multisig: {config.multisig.owners.length} owners, {config.multisig.threshold} confirmations</span>
                      {isClient && (
                        <ReactJsonView
                          src={{
                            owners: config.multisig.owners,
                ...
                {selectedChains.length > 0 && (
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <span className="font-bold">chains: {selectedChains.length}</span>
                      {isClient && (
                        <ReactJsonView
                          src={selectedChains}
                          theme="bright:inverted"
